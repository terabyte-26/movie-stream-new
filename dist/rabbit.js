...
const main = async (embed_url, site) => {
    referrer = site;
    ...
    return {
        getResources: resp_json,
        real: real
    };
    console.log("123", real);
};
// main("https://megacloud.tube/embed-1/e-1/OCEGWE11DkO9?z=", "https://sflix.to/"); //change this value to the embed_url you want
...
app.post('/process', async (req, res) => {
    const { embed_url, site } = req.body;
    if (!embed_url || !site) {
        return res.status(400).json({ error: "Missing required parameters: embed_url and site." });
    }
    try {
        const result = await main(embed_url, site);
        res.json(result);
    }
    catch (error) {
